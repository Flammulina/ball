...
void CPQ2016_GRAPHDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ANIMATE1, m_xc_Animation);
	DDX_Control(pDX, IDC_GRAPH, m_xcdc);
}
...
//	ON_BN_CLICKED(IDC_BUTTON2, &CPQ2016_GRAPHDlg::OnBnClickedButton2)
	...
	*/
	...
	ON_BN_CLICKED(IDOK, &CPQ2016_GRAPHDlg::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL, &CPQ2016_GRAPHDlg::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_BUTTON1, &CPQ2016_GRAPHDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CPQ2016_GRAPHDlg::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON3, &CPQ2016_GRAPHDlg::OnBnClickedButton3)
	ON_BN_CLICKED(IDC_BUTTON4, &CPQ2016_GRAPHDlg::OnBnClickedButton4)
	ON_ACN_STOP(IDC_ANIMATE1, &CPQ2016_GRAPHDlg::OnAcnStopAnimate1)
	ON_BN_CLICKED(IDC_BUTTON5, &CPQ2016_GRAPHDlg::OnBnClickedButton5)
END_MESSAGE_MAP()
...
BOOL CPQ2016_GRAPHDlg::OnInitDialog()
{
	...
	*/
	...
	PlaySound(_T("start.wav"), NULL,  SND_ASYNC||SND_LOOP);
...
void CPQ2016_GRAPHDlg::OnBnClickedOk()
{
...
void CPQ2016_GRAPHDlg::OnBnClickedCancel()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	CDialog::OnCancel();
}
...
void CPQ2016_GRAPHDlg::OnBnClickedButton1()
{
	...
	m_xc_Animation.ShowWindow(true);
	m_xc_Animation.Open(L"countGo!.avi");
	m_xc_Animation.Play(0, -1, 1);
	
	//m_xc_Animation.Stop();
}


void CPQ2016_GRAPHDlg::OnAcnStopAnimate1()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	srand(time(NULL));
	if (check == 0){
		m_xc_Animation.ShowWindow(false);
		// START
		PlaySound(_T("10sec.wav"), NULL, SND_ASYNC);
		int r = rand() % 3;
		if (r==0){
			m_xc_Animation.Open(L"play1.avi");
			m_xc_Animation.ShowWindow(true);
			m_xc_Animation.Play(0, -1, 1);
		}
		else if (r == 1){
			m_xc_Animation.Open(L"play2.avi");
			m_xc_Animation.ShowWindow(true);
			m_xc_Animation.Play(0, -1, 1);
		}
		else if (r == 2){
			m_xc_Animation.Open(L"play3.avi");
			m_xc_Animation.ShowWindow(true);
			m_xc_Animation.Play(0, -1, 1);
		}
...
void CPQ2016_GRAPHDlg::OnBnClickedButton3()
{
	...
	for (i = 0; i < 1000; i++){
		...
		*/
			...
			// SEQ, Ax, Ay, Az, Gx, Gy, Gz　in integer
...
void CPQ2016_GRAPHDlg::OnBnClickedButton4()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	MessageBox(L"\nおどれ",L"あそびかた");
}

void CPQ2016_GRAPHDlg::OnBnClickedButton5()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	int s=0;
	double t = 0, u = 0;
	...
	fprintf(fo,"ay:%lf\taz:%lf\ndx:%lf\tdy:%lf\tdz:%lf",v1,v2,v3,v4,v5);
	fclose(fo);
	m_xc_Animation.ShowWindow(true);
	m_xc_Animation.Stop();
	if (ball==0){
		m_xc_Animation.Open(L"tabletennis.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 1){
		m_xc_Animation.Open(L"tennis.avi");//tenn
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 2){
		m_xc_Animation.Open(L"rag.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 3){
		m_xc_Animation.Open(L"basket.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 4){
		m_xc_Animation.Open(L"soccer.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 5){
		m_xc_Animation.Open(L"baseball.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 6){
		m_xc_Animation.Open(L"kamechan.avi");
		m_xc_Animation.Play(0, -1, -1);
	}
	else if (ball == 7){
		m_xc_Animation.Open(L"sustainable.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 8){
		m_xc_Animation.Open(L"bird.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 9){
		m_xc_Animation.Open(L"ds.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 10){
		m_xc_Animation.Open(L"ra.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else if (ball == 11){
		m_xc_Animation.Open(L"hougan.avi");
		m_xc_Animation.Play(0, -1, 1);
	}
	else{
		m_xc_Animation.Open(L"soccer.avi");
